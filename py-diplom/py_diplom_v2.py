...
class UserVK:
    ...
    def get_groups(self):
        """
        Функция возвращает количество групп у пользователя,
        список id групп пользователя запоминается в атрибуте экземпляра класса.
        :return:
        """
        self.params['user_id'] = self.user_id
        response = requests.get('{0}groups.get'.format(self.url), self.params)
        try:
            self.user_groups = response.json()['response']['items']
            return 'count', response.json()['response']['count']
        except KeyError:
    ...
    def get_is_member(self, group):
        """
        Функция проверяет, являются ли друзья ползователя членами данной группы.
        ...
        self.params['user_ids'] = ','.join(map(str, self.user_friends))
        response = requests.get('{0}groups.isMember'.format(self.url), self.params)
        i = False
        try:
            result = response.json()['response']
            for item in result:
            ...
            return 'Error', f'Не удалось получить членов группы {group}. Результат будет не точен.'
        finally:
            del self.params['user_ids']
...
class MainForm(npyscreen.ActionForm):
    ...
    def get_diff_groups(self):
        ...
        :return:
        """
        a = round(97 / len(self.user.user_groups), 2)
        secret_groups = []
        for group in self.user.user_groups:
            self.text4.value = f'Проверяется группа {group}'
            self.text4.display()
            result = self.user.get_is_member(group)
            if result[0] == 'Error':
                self.text6.value = result[1]
            elif not result[1]:
                secret_groups.append(group)
                self.text5.value = f'Группы: {secret_groups}'
            else:
                self.text6.value = f'Группа {group} является общей.'
                self.text6.display()
        ...
        secret_str = ','.join(map(str, secret_groups))
